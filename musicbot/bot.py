...
class MusicBot(discord.Client):
    ...
    async def cmd_pn(self, player, channel, author, permissions, leftover_args, song_url):
        """
        Usage:
           {command_prefix}pn song_link
           {command_prefix}pn {text to search for}
        """
        result = await self.cmd_playnext(player, channel, author, permissions, leftover_args, song_url)
        return result
    ...
    async def cmd_playnext(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if 'entries' in info:
            ...
            try:
                entry, position = await player.playlist.add_entry_top(song_url, channel=channel, author=author)
    ...
    async def cmd_q(self, channel, player):
        ...
        """
        result = await self.cmd_queue(channel, player)
        return result
